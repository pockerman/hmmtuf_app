...
template = {'legend.numpoints': 1,
            ...
            'axes.grid': False, 'ytick.minor.size': 0.0, 'legend.scatterpoints': 1,
            'axes.edgecolor': "black", 'grid.color': 'white', 'legend.frameon': False,
            'ytick.color': '.15', 'xtick.major.size': 4.0, 'xtick.major.width': 1.0,
            'figure.facecolor': "#EAEAF2", 'xtick.color': '.15', 'xtick.minor.size': 3.0,
            'xtick.direction': u'out', 'lines.solid_capstyle': u'round',
            'grid.linestyle': u'-', 'image.cmap': u'Greys', 'axes.facecolor': "white", 'text.color': '.15',
            'ytick.direction': u'out', 'axes.linewidth': 1.0}
...
def make_figure(peak_matrix_list, kmer_count_signal, repeat_unit_array,
                gap_repeat_unit_array, aln_repeat_unit_array,
                unit_length_list, matrix_sumx_list, periods,
                candidate, k=10, thresh=20, peak_period_list=[],
                xlabel_a="Position (bp)", fig_name="hoge", key="",
                gb="None",  dtype="DNA", p_start=0, p_end=0, Format="pdf", html_path="../../../../script/html", strand=0):
...
def motif_logo(dtype, path):
    fasta_name = "align.unit_seq.fasta"
    fasta = open(path + fasta_name)
    ...
    if dtype == "nucl":
        ...
        try:
            ...
            fout = open(path + "weblogo.eps","wb")
            fout.write(eps_formatter(data, format))
            fout.close() 
        try:
            fout = open(path + "weblogo.txt","wb")
...
def load_data(dtype, strand, ksize ,thresh, path, Format="pdf"):
    ...
    if dtype == "nucl":
        make_figure(peak_matrix_list, kmer_count_signal, rpt_unit_array_list, gap_rpt_unit_array_list,
                    aln_rpt_unit_array_list, unit_length_list, avg_intensity_list,
                    periods, candidate, peak_period_list=peak_list, gb=gbk,
                    fig_name="periodic_repeat", thresh=thresh, dtype=dtype, Format=Format, k=ksize, strand=strand)
    elif dtype == "prot":
        make_figure(peak_matrix_list, kmer_count_signal, rpt_unit_array_list,
                    gap_rpt_unit_array_list, aln_rpt_unit_array_list, unit_length_list,
                    avg_intensity_list, periods, candidate, peak_period_list=peak_list,
                    gb=gbk, fig_name="periodic_repeat", thresh=thresh,
                    dtype=dtype ,Format=Format, p_start=p_start, p_end=p_end, k=ksize, strand=strand)
    return peak_list
